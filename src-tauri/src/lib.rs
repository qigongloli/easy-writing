...
fn create_external_webview_window(
    _app: &DesktopAppHandle,
    _target: &str,
    _title: Option<String>,
    _features: Option<NewWindowFeatures>,
) -> Result<DesktopWebviewWindow, String> {
    // 安卓不支持桌面多窗口/外部 WebView 弹窗，直接返回错误，由前端走外部浏览器打开。
    Err("移动端不支持应用内打开网页，已改用系统浏览器".to_string())
}
...
async fn rank_crawl_render_page(
    _app: DesktopAppHandle,
    _request: RankCrawlRequest,
) -> Result<String, String> {
    // 榜单抓取依赖桌面隐藏窗口加载网页，移动端不支持，前端应隐藏该功能入口。
    Err("榜单抓取仅支持桌面版".to_string())
}
#[cfg(not(target_os = "android"))]
#[tauri::command]
async fn rank_crawl_render_page(
...
/// 安卓移动端：不手动建窗口，由系统按启动入口创建；这里复用默认标签获取主窗口。
#[cfg(target_os = "android")]
fn build_main_window(app: &DesktopAppHandle) -> tauri::Result<DesktopWebviewWindow> {
...
pub fn run() {
    ...
    // 单实例插件仅桌面端支持，移动端（安卓/iOS）由系统接管应用生命周期。
    #[cfg(not(target_os = "android"))]
    {
